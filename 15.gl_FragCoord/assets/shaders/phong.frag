...
void main()
{
        ...
        // test, gl_FragCoord代表屏幕的意思， 例如屏幕大小为1600 1200
        // 下面的意思代表左边800 * 1200红色，右边为蓝色
        float flag = step(800, gl_FragCoord.x);
        // (1 - flag) * param1 + param2 * flag
        vec3 blendColor = mix(vec3(1.0f, 0.0f, 0.0f), vec3(0.0f, 0.0f, 0.1f), flag);

        // 最终计算
        fragColor = vec4(finalColor * blendColor, 1.0);
}