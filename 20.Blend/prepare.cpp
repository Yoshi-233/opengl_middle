...
void prepareAll()
{
        ...
        auto boxMarterial = std::make_shared<PhongMaterial>();
        boxMarterial->setDiffuse(std::make_shared<Texture>("assets/textures/box.png", 0));
        auto boxMesh = std::make_shared<Mesh>(boxGeo, boxMarterial);
        scene->addChild(boxMesh);

        /* plane */
        auto planeGeo = Geometry::createPlane(6.0f, 6.0f);
        auto planeMaterial = std::make_shared<PhongMaterial>();
        planeMaterial->setDiffuse(std::make_shared<Texture>("assets/textures/wall.jpg", 0));
        auto planeMesh = std::make_shared<Mesh>(planeGeo, planeMaterial);
