...
void prepareAll()
{
        ...
        /* 由于深度检测的存在，如果先颜色混合或者再深度检测，会导致后面的物体不显示 */
        auto model = AssimpLoader::loadModel("assets/fbx/bag/backpack.obj");
        setModelBlend(model, true, 0.5f);
        scene->addChild(model);

        /* 实体平面 */
        auto planeGeo1 = Geometry::createPlane(5.0f, 5.0f);
        auto planeMat1 = std::make_shared<PhongMaterial>();
        planeMat1->setDiffuse(std::make_shared<Texture>("assets/textures/box.png", 0));
        auto plane1 = std::make_shared<Mesh>(planeGeo1, planeMat1);
        plane1->setPosition(glm::vec3(0.0f, 0.0f, 6.0f));
        scene->addChild(plane1);

        /* 半透明平面 */
        auto planeTransGeo = Geometry::createPlane(10.0f, 10.0f);
        auto planeTransMat = std::make_shared<PhongMaterial>();
        planeTransMat->setDiffuse(std::make_shared<Texture>("assets/textures/wall.jpg", 0));
        planeTransMat->mBlend = true;
        planeTransMat->mOpacity = 0.4f;
        auto planeTrans = std::make_shared<Mesh>(planeTransGeo, planeTransMat);
        planeTrans->setPosition(glm::vec3(0.0f, 0.0f, -6.0f));
        scene->addChild(planeTrans);

        /* 实体平面 */
        auto planeGeo2 = Geometry::createPlane(10.0f, 10.0f);
        auto planeMat2 = std::make_shared<PhongMaterial>();
        planeMat2->setDiffuse(std::make_shared<Texture>("assets/textures/goku.jpg", 0));
        auto plane2 = std::make_shared<Mesh>(planeGeo2, planeMat2);
        plane2->setPosition(glm::vec3(3.0f, 0.0f, 0.0f));
        plane2->rotateY(45.0f);
