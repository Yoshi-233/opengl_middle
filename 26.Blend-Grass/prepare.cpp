...
void prepareAll()
{
        ...
        auto grassModel = AssimpLoader::loadModel("./assets/fbx/grass.fbx");
        grassModel->setScale(glm::vec3(0.02f));
        grassModel->rotateZ(90.0f);
        scene->addChild(grassModel);

        /* 全局材质 */
        auto grassMaterial = std::make_shared<OpacityMaskMaterial>();
        grassMaterial->setDiffuse(std::make_shared<Texture>("./assets/textures/grass.jpg", 0));
        grassMaterial->setOpacityMask(std::make_shared<Texture>("./assets/textures/grassMask.png", 1));
