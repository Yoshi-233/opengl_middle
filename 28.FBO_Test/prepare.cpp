...
//TODO:
unsigned int fbo;
std::shared_ptr<Texture> colorAttachement;
...
void prepareFBO()
{
        ...
        /* 2.创建颜色附件，加入fbo */
        colorAttachement = std::make_shared<Texture>(0, SCREEN_WIDTH, SCREEN_HEIGHT);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                               colorAttachement->getTextureID(), 0);
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, APP.getWidth(), APP.getHeight(),
...
void prepareAll()
{
        ...
        boxMat->setDiffuse(std::make_shared<Texture>("assets/textures/grass.jpg", 0));
        auto boxMesh = std::make_shared<Mesh>(boxGeo, boxMat);
        sceneOffScreen->addChild(boxMesh);

        // 贴屏渲染的box
        auto planeGeo1 = Geometry::createScreenPlane();
        auto planeMat1 = std::make_shared<ScreenMaterial>();
        /* 这一步很重要， 因为我们要将贴图渲染到屏幕上，
         * 第一帧渲染后的纹理会被缓存到贴图中，下次渲染时直接使用缓存的纹理，而不是重新渲染 */
        planeMat1->mScreenTexture = colorAttachement;
        auto plane1 = std::make_shared<Mesh>(planeGeo1, planeMat1);
        sceneInScreen->addChild(plane1);
