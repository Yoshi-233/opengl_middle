...
public:
        ...
        template<class mat_type>
        void setMatrixArray(const std::string &name, mat_type *array, int count) const;
...
template<class mat_type>
void Shader::setMatrixArray(const std::string &name, mat_type *array, int count) const
{
        GLint location = GL_CHECK_ERR(glGetUniformLocation(this->mProgram, name.c_str()));
         ...
         * */
        if (std::is_same<mat_type, glm::mat4>::value) {
                GL_CHECK_ERR(glUniformMatrix4fv(location, count,
                                                GL_FALSE, glm::value_ptr(array[0])));
        } else if (std::is_same<mat_type, glm::mat3>::value) {
                GL_CHECK_ERR(glUniformMatrix3fv(location, count,
                                                GL_FALSE, glm::value_ptr(array[0])));
        } else if (std::is_same<mat_type, glm::mat2>::value) {
                GL_CHECK_ERR(glUniformMatrix2fv(location, count,
                                                GL_FALSE, glm::value_ptr(array[0])));
        } else if (std::is_same<mat_type, glm::mat4x2>::value) {
                GL_CHECK_ERR(glUniformMatrix4x2fv(location, count,
                                                  GL_FALSE, glm::value_ptr(array[0])));
        } else if (std::is_same<mat_type, glm::mat4x3>::value) {
                GL_CHECK_ERR(glUniformMatrix4x3fv(location, count,
                                                  GL_FALSE, glm::value_ptr(array[0])));
        } else if (std::is_same<mat_type, glm::mat3x2>::value) {
                GL_CHECK_ERR(glUniformMatrix3x2fv(location, count,
                                                  GL_FALSE, glm::value_ptr(array[0])));
        } else if (std::is_same<mat_type, glm::mat3x4>::value) {
                GL_CHECK_ERR(glUniformMatrix3x4fv(location, count,
                                                  GL_FALSE, glm::value_ptr(array[0])));
        } else if (std::is_same<mat_type, glm::mat2x3>::value) {
                GL_CHECK_ERR(glUniformMatrix2x3fv(location, count,
                                                  GL_FALSE, glm::value_ptr(array[0])));
        } else if (std::is_same<mat_type, glm::mat2x4>::value) {
                GL_CHECK_ERR(glUniformMatrix2x4fv(location, count,
                                                  GL_FALSE, glm::value_ptr(array[0])));
        } else {
                ERROR("type error!");
        }
