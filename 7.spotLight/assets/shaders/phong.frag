...
void main()
{
        ...
        // float cVisiable = cos(innerAngle); // glsl这里传入的是弧度
        // float spotFlag = step(cVisiable, cTheta); // 只有cTheta > cVisiable时才返回1，否则返回0
        float cGamma = dot(lightDirN, targetDirN);
        float spotIntensity = clamp((cGamma - outerLine) / (innerLine - outerLine), 0.0f, 1.0f);
