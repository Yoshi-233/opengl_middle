        ...
        for (const auto &mesh: meshes) {
                ...
                switch (material->mType) {
                        case MaterialType::PhongMaterial: {
                                ...
                                // 光源参数更新
                                shaderPtr->setVectorFloat("lightPosition", spotLight->getPosition());
                                shaderPtr->setVectorFloat("lightColor", spotLight->getColor());
                                shaderPtr->setFloat("specularIntensity", spotLight->getSpecularIntensity());
                                shaderPtr->setVectorFloat("ambientColor", ambientLight->getColor());
                                shaderPtr->setVectorFloat("cameraPosition", camera->mPosition);
                                shaderPtr->setFloat("shiness", phongMaterial->getShiness());

                                // 聚光灯参数
                                shaderPtr->setVectorFloat("targetDirection", spotLight->getTargetDirection());
                                // 下面是弧度值
                                shaderPtr->setFloat("innerLine", glm::cos(glm::radians(spotLight->getInnerAngle())));
                                shaderPtr->setFloat("outerLine", glm::cos(glm::radians(spotLight->getOuterAngle())));
                        }
                                break;
                        case MaterialType::WhiteMaterial: {
                                shaderPtr->setMatrix<decltype(mesh->getModelMatrix())>("modelMatrix",
