...
void Renderer::render(const std::vector<std::shared_ptr<Mesh>> &meshes, const std::shared_ptr<Camera> &camera,
                      const std::shared_ptr<DirectionalLight> &directionalLight, const std::shared_ptr<PointLight> &pointLight,
                      const std::shared_ptr<SpotLight> &spotLight, const std::shared_ptr<AmbientLight> &ambientLight)
{
        /* 1. opengl的必要状态机 */
        glEnable(GL_DEPTH_TEST);
        glDepthFunc(GL_LESS);
        ...
        for (const auto &mesh: meshes) {
                ...
                switch (material->mType) {
                        case MaterialType::PhongMaterial: {
                                ...
                                /* 点光源参数 */
                                shaderPtr->setVectorFloat("pointLight.position", pointLight->getPosition());
                                shaderPtr->setVectorFloat("pointLight.color", pointLight->getColor());
                                shaderPtr->setFloat("pointLight.specularIntensity", pointLight->getSpecularIntensity());
                                ...
                                /* 平行光参数 */
                                shaderPtr->setVectorFloat("directionalLight.direction", directionalLight->getDirection());
                                shaderPtr->setVectorFloat("directionalLight.color", directionalLight->getColor());
                                shaderPtr->setFloat("directionalLight.specularIntensity", directionalLight->getSpecularIntensity());
                                ...
                                shaderPtr->setFloat("spotLight.specularIntensity", spotLight->getSpecularIntensity());
                                shaderPtr->setVectorFloat("spotLight.targetDirection", spotLight->getTargetDirection());
                                // 下面是弧度值
                                shaderPtr->setFloat("spotLight.innerLine", glm::cos(glm::radians(spotLight->getInnerAngle())));
                                shaderPtr->setFloat("spotLight.outerLine", glm::cos(glm::radians(spotLight->getOuterAngle())));
                        }
                                break;
                        case MaterialType::WhiteMaterial: {
                                shaderPtr->setMatrix<decltype(mesh->getModelMatrix())>("modelMatrix",
